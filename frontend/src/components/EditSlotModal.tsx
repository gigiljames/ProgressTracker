...
import { editSlot } from "../api/slotService";
...
function EditSlotModal({
  ...
  // Populate fields whenever the slot prop changes or modal opens
  useEffect(() => {
    if (isOpen && slot) {
      setDate(slot.date);
  ...
  function handleSubmit() {
    ...
    editSlot(slot._id, {
      ...
      .catch((e: any) => {
        toast.error(
          e?.response?.data?.message || e?.message || "Failed to update slot.",
        );
      })
